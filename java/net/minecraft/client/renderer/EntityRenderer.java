...
import xu.zhixuan.core.modules.Perspective;
import xu.zhixuan.core.modules.Zoom;
import xu.zhixuan.wulne.UI.Client.GuiMainMenu;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private float zoomModifierHandPrev = 1.0F;
    private float zoomModifierHand = 1.0F;
    ...
    private void updateZoomModifierHand() {
        float f = 1.0F;
        if (Config.zoomMode) {
            f = 4.0F;
        }

        this.zoomModifierHandPrev = this.zoomModifierHand;
        this.zoomModifierHand += (f - this.zoomModifierHand) / 2.0F;
        if (this.zoomModifierHandPrev + 0.01F > 4.0F) {
            this.zoomModifierHandPrev = 4.0F;
        }

        if (this.zoomModifierHand + 0.01F > 4.0F) {
            this.zoomModifierHand = 4.0F;
        }

        if (this.zoomModifierHandPrev - 0.01F < 1.0F) {
            this.zoomModifierHandPrev = 1.0F;
        }

        if (this.zoomModifierHand - 0.01F < 1.0F) {
            this.zoomModifierHand = 1.0F;
        }
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        ...
        float f1;
        Block block;
        if (WulneJvav.INSTANCE.moduleManager.getModByClass(Zoom.class).isEnabled()) {
            if (this.debugView) {
                return 90.0F;
            } else {
                ...
                f /= this.zoomModifierHandPrev + (this.zoomModifierHand - this.zoomModifierHandPrev) * partialTicks;
                if (flag) {
                    if (!Config.zoomMode) {
                ...
                if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).getHealth() <= 0.0F) {
                    f1 = (float)((EntityLivingBase)entity).deathTime + partialTicks;
                    f /= (1.0F - 500.0F / (f1 + 500.0F)) * 2.0F + 1.0F;
                }

                block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping())
        {
            ...
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                GlStateManager.rotate(
                        Perspective.getCameraPrevYaw()
                                + (Perspective.getCameraYaw() - Perspective.getCameraPrevYaw()) * partialTicks + 180.0F,
                        0.0F, -1.0F, 0.0F);
                GlStateManager.rotate(
                        Perspective.getCameraPrevPitch()
                                + (Perspective.getCameraPitch() - Perspective.getCameraPrevPitch()) * partialTicks,
                        -1.0F, 0.0F, 0.0F);
            }
        }
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists())
        {
            if (!this.mc.gameSettings.debugCamEnable)
            {
                float f6 = Perspective.getCameraPrevYaw()
                        + (Perspective.getCameraYaw() - Perspective.getCameraPrevYaw()) * partialTicks + 180.0F;
                float f7 = Perspective.getCameraPrevPitch()
                        + (Perspective.getCameraPitch() - Perspective.getCameraPrevPitch()) * partialTicks;
                float f8 = 0.0F;
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        if (Perspective.overrideMouse() && flag) {
