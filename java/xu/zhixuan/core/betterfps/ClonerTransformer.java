...
import xu.zhixuan.wulne.Api.Minecraft.Launcher.IClassTransformer;
...
public class ClonerTransformer implements IClassTransformer {
        ...
        for(Clone c : clones) {
            ...
            try {
                ...
                fields: for(FieldNode field : cloneClass.fields) {
                    CopyMode.Mode mode = CopyMode.Mode.REPLACE;
                    Naming name = null;
    ...
    private CopyMode.Mode getCopyMode(List<AnnotationNode> annotations) {
        for(AnnotationNode node : annotations) {
            if(node.desc.equals(Type.getDescriptor(CopyMode.class))) {
                CopyMode.Mode n = ASMUtils.getAnnotationValue(node, "value", CopyMode.Mode.class);
                if(n != null) return n;
            }
        }
    ...
    private void cloneField(FieldNode e, ClassNode node, CopyMode.Mode mode, Naming name) {
        if(mode == CopyMode.Mode.IGNORE) return;
        for(int i = 0; i < node.fields.size(); i++) {
            ...
            if(b) {
                if(mode == CopyMode.Mode.ADD_IF_NOT_EXISTS) return;
    ...
    private boolean cloneMethod(MethodNode e, ClassNode node, ClassNode original, CopyMode.Mode mode, Naming name) {
        if(mode == CopyMode.Mode.IGNORE) return false;
        ...
        for(int i = 0; i < node.methods.size(); i++) {
            ...
            if(b) {
                if(mode == CopyMode.Mode.ADD_IF_NOT_EXISTS) return false;
